...
Page({
  ...
  onLoad: function (options) {
    console.log(options)
    let that = this
    let post_id = options.goodsid;
    let shopCode = options.shopCode;
    that.setData({
      goods_id: post_id,
    })
    function goods(res) {
      ...
      for (let xx = 0; xx < xsort.length; xx++) {
        for (let qq = 0; qq < xpicture.length; qq++) {
    ...
    utilFunctions.getGoodsDetails(that.data.goods_id, shopCode, goods, this)
